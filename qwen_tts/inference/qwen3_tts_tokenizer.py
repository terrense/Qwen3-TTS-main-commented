...
"""
Qwen3 语音 tokenizer 的高层推理包装。

模块定位:
- 这个类的职责是把各种音频输入形式整理成 tokenizer 模型可接受的张量，
  并把底层 `encode/decode` 的细节收敛成统一接口。

AI 视角:
- `encode` 把连续波形压缩成离散声学 token，便于传输、存储和后续语言模型建模。
- `decode` 把离散 token 还原回波形，是声码器侧的逆过程。
- 25Hz 与 12Hz 两个 tokenizer 版本在条件信息上不同:
  25Hz 额外需要 x-vector 与参考 mel，
  12Hz 则主要围绕多码本离散 token 本身工作。

Python 视角:
- 这个包装类最核心的价值不是“算法”，而是“接口鲁棒性”:
  统一处理路径、URL、base64、numpy、批量输入、设备迁移和 padding。
"""
...
class Qwen3TTSTokenizer:
    """
    Qwen3 语音 tokenizer 的统一封装。

    提供能力:
    - `from_pretrained()`:
      同时加载底层 tokenizer 模型和 `AutoFeatureExtractor`。
    - `encode()`:
      支持 wav 路径、URL、base64、numpy 波形及其批量形式。
    - `decode()`:
      支持直接吃 `encode()` 的原始输出，也支持简化字典格式。

    使用原则:
    - 传 numpy 时必须提供原始采样率 `sr`，因为 tokenizer 的输入采样率是固定的。
    - 所有解码结果最终都会标准化为 `float32 numpy` 波形以及一个输出采样率整数。
    """
    ...
    def _is_probably_base64(self, s: str) -> bool:
        """用启发式规则区分长字符串是 base64 音频还是普通文件路径。"""
    ...
    def _normalize_audio_inputs(
        ...
        if isinstance(audios[0], str):
            # 路径、URL、base64 都统一经 `load_audio` 读取并重采样。
    ...
    def encode(
        ...
        with torch.inference_mode():
            # feature extractor 输出常带一个多余通道维，底层 `model.encode` 只需要 `(B, T)`。
    ...
    def decode(
        ...
        # 先把多种输入形式规整成和官方 demo 相同的内部结构，后面 decode 逻辑就能保持单一路径。
        ...
        if isinstance(audio_codes_list, torch.Tensor):
            # 兼容“单样本 tensor”与“已 pad 的 batch tensor”两类情况。
            t = audio_codes_list
            if t.dim() == 1:
                # 25Hz 单样本: `(codes_len,) -> (1, codes_len)`
                t = t.unsqueeze(0)
            elif t.dim() == 2:
                # 12Hz 单样本: `(codes_len, num_quantizers) -> (1, codes_len, num_quantizers)`
                t = t.unsqueeze(0)
        ...
        with torch.inference_mode():
            if model_type == "qwen3_tts_tokenizer_25hz":
                ...
                # 12Hz 解码只依赖多码本离散 token，不需要额外说话人条件。
