...
"""
面向最终用户的 Qwen3-TTS 推理包装层。

模块定位:
- `core/models/modeling_qwen3_tts.py` 是真正的神经网络实现；
  本文件则是“工程可用接口层”，负责把真实世界的输入形态
  (字符串、URL、base64、numpy 波形、批量列表) 归一化成模型能接受的张量。

AI 视角:
- 这个包装层把三类任务统一起来:
  1. `generate_custom_voice`: 固定说话人 + 文本 + 可选风格指令。
  2. `generate_voice_design`: 文本 + 自然语言音色描述。
  3. `generate_voice_clone`: 参考音频驱动的声音克隆。
- 对于 Base 模型，最关键的工作是把参考音频拆成两种条件:
  离散语音码 `ref_code` 和说话人向量 `ref_spk_embedding`。

Python/工程视角:
- 该类模仿 Hugging Face `from_pretrained` 风格，降低使用门槛。
- 大量辅助函数本质是在做“输入规范化”和“批处理维度对齐”，
  这是 Python 推理服务代码里最容易出错、也最值得写清楚的部分。
"""
...
class VoiceClonePromptItem:
    ...
    AI 视角:
    - `ref_code` 是参考语音经过 speech tokenizer 离散化后的声学条件，
      提供“怎么说”的局部模式。
    - `ref_spk_embedding` 是 speaker encoder 提取的说话人表示，
      提供“谁在说”的全局音色条件。
    - `icl_mode` 与 `x_vector_only_mode` 决定模型是做完整上下文模仿，
      还是只借用说话人身份向量。

    Python 视角:
    - 使用 `@dataclass` 可以自动生成 `__init__`、`__repr__` 等样板代码，
      非常适合这种以字段为核心的数据承载对象。
    - 字段设计与 `Qwen3TTSForConditionalGeneration.generate(..., voice_clone_prompt=...)`
      的输入格式保持对齐，便于在包装层和底层模型之间无损传递。
    """
...
class Qwen3TTSModel:
    ...
    提供能力:
    - `from_pretrained()`:
      以 Hugging Face 风格加载模型、processor 与 generation 配置。
    - 任务接口:
      `generate_custom_voice()`、`generate_voice_design()`、
      `create_voice_clone_prompt()`、`generate_voice_clone()`。
    - 统一输出:
      所有生成接口最终都返回 `(wavs: List[np.ndarray], sample_rate: int)`。

    设计重点:
    - 这个类不重新实现神经网络，而是把“多种现实输入形态”映射到底层模型接口。
    - 语言、speaker 合法性校验放在这里做，而不是拖到模型前向时才失败，
      这是典型的工程防御式编程。
    """
    ...
    def _is_probably_base64(self, s: str) -> bool:
        """用启发式规则判断字符串更像 base64 音频而不是文件路径。"""
    ...
    def _decode_base64_to_wav_bytes(self, b64: str) -> bytes:
        """兼容 data URL 与纯 base64 两种常见 Web 音频上传格式。"""
        if "," in b64 and b64.strip().startswith("data:"):
            b64 = b64.split(",", 1)[1]
        return base64.b64decode(b64)

    def _load_audio_to_np(self, x: str) -> Tuple[np.ndarray, int]:
        """把路径、URL 或 base64 音频统一读成单声道 `float32` numpy 波形。"""
        if self._is_url(x):
    ...
    def create_voice_clone_prompt(
        ...
        for i, ((wav, sr), code, rtext, xvec_only) in enumerate(zip(normalized, ref_codes, ref_text_list, xvec_list)):
            ...
            if sr != self.model.speaker_encoder_sample_rate:
                # speaker encoder 对采样率是强约束，必须对齐到训练时使用的 24kHz。
    ...
    def _prompt_items_to_voice_clone_prompt(self, items: List[VoiceClonePromptItem]) -> Dict[str, Any]:
        """把 dataclass 列表转换成底层 `generate` 所需的字段字典。"""
        return dict(
    ...
    def generate_voice_clone(
        ...
        **kwargs,
        ...
        if voice_clone_prompt is None:
            ...
            # 用户只给了参考音频时，先构造可复用的 prompt，再进入统一生成路径。
        ...
        if ref_texts_for_ids is not None:
            ...
            for i, rt in enumerate(ref_texts_for_ids):
                if rt is None or rt == "":
                    ...
                    # ICL 模式下，参考转写文本会与参考 codec 一起拼成模型前缀。
