...
模块总览:
- 这是仓库中最核心的神经网络实现文件，负责定义 Qwen3-TTS 的主生成模型。
- 从工程分层上看，它位于“底层网络层”，上面由 `qwen_tts.inference.qwen3_tts_model`
  做用户接口封装，下面则依赖 speech tokenizer 负责声学离散码与波形互转。

AI 架构视角:
- 整体可以粗略拆成四块:
  1. `Qwen3TTSSpeakerEncoder`: 从参考 mel 中提取说话人向量。
  2. `Qwen3TTSTalkerModel`: 主 talker，自回归地产生 codec-0 主通道。
  3. `Qwen3TTSTalkerCodePredictorModel`: 根据主通道隐藏状态补齐其余 codec 组。
  4. `Qwen3TTSForConditionalGeneration`: 将 talker、speaker encoder、speech tokenizer
     组装成完整的 TTS 生成系统。
- 这种设计的关键点在于“文本语义建模”和“多码本声学建模”并没有完全分开，
  而是通过统一隐藏状态在同一生成框架中耦合。

Python/实现视角:
- 这里大量继承 Hugging Face 的 `PreTrainedModel`、`GenerationMixin`，
  目的是直接接入 `from_pretrained`、`generate`、cache、attention backend 等通用能力。
- 对阅读者而言，最值得关注的不是每一层线性变换，而是:
  1. 输入是如何被拼成统一序列的；
  2. attention mask 与 rotary embedding 如何配合；
  3. 多码本 codec 的主通道和辅通道如何分阶段生成。
"""
...
class Qwen3TTSSpeakerEncoder(torch.nn.Module):
    ...
    AI 视角:
    - 该模块基于 ECAPA-TDNN 思路，把参考语音的 mel 谱压缩成一个固定维度的
      speaker embedding。
    - 这个向量表达的是“说话人身份/音色”，而不是具体的文本内容，
      因此适合作为 voice clone 与 custom voice 的全局条件。

    张量视角:
    - 输入通常是 `(batch, mel_frames, mel_bins)`。
    - 输出是 `(batch, speaker_dim)` 的全局说话人向量。

    Python 视角:
    - 这里大量使用 `nn.ModuleList` 拼装重复结构，
      方便根据配置动态扩展层数与通道数。
    """
...
class Qwen3TTSTalkerAttention(nn.Module):
    ...
    AI 视角:
    - talker 负责在文本条件下生成主 codec 通道，因此它的注意力层既要保留语言建模能力，
      又要能处理声学 token 的时间结构。
    - 这里使用带 RoPE 的自注意力，并兼容 grouped-query attention 与滑动窗口。

    Python 视角:
    - `forward` 里最关键的几步是:
      1. 线性投影得到 Q/K/V；
      2. 应用 rotary 位置编码；
      3. 根据配置选择 eager / sdpa / flash-attn 后端；
      4. 再映射回隐藏维度。
    """
...
class Qwen3TTSAttention(nn.Module):
    """Code predictor 使用的标准自注意力层。

    与 `Qwen3TTSTalkerAttention` 的区别:
    - 该注意力主要服务于“给定主通道隐藏状态后，预测其余 codec 组”的过程，
      因此使用的是更标准的单轴自回归 RoPE 位置编码。
    """
...
class Qwen3TTSDecoderLayer(GradientCheckpointingLayer):
    """Code predictor 的基础 Transformer block。

    结构顺序:
    - RMSNorm -> Self-Attention -> Residual
    - RMSNorm -> MLP -> Residual

    这与当前主流 decoder-only LLM 的 block 结构保持一致，
    便于复用 HF 侧的 cache 和 attention 基础设施。
    """
    def __init__(self, config: Qwen3TTSConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Self Attention: 先做因果注意力，让当前位置只能看到历史文本和历史 codec 条件。
        ...
        # Fully Connected: 用门控 MLP 做逐位置非线性变换，再叠加残差。
        residual = hidden_states
...
class Qwen3TTSTalkerCodePredictorModel(Qwen3TTSPreTrainedModel):
    """预测次级 codec 组隐藏状态的解码器。

    AI 视角:
    - Qwen3-TTS 把 codec-0 视为主生成通道，其余 codec 组由一个更小的 predictor
      在条件隐藏状态上补全。
    - 这样做能降低主模型的生成难度，同时保留多码本重建质量。

    Python 视角:
    - 该模块只接受 `inputs_embeds`，不直接接 token id，
      因为上游已经把主 talker 产生的隐藏状态和其他 codec embedding 融合好了。
    """
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        # `attention_mask` 既可能来自调用方，也可能需要在这里按配置即时构造。
...
class Qwen3TTSTalkerCodePredictorModelForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    """面向生成接口封装过的 sub-talker / code predictor。

    职责拆分:
    - `forward_finetune`:
      用于训练阶段，一次性预测多个 codec 组并计算联合损失。
    - `forward`:
      用于推理阶段，按 generation step 逐组预测下一个 codec 分量。

    这两个入口的差异，正体现了训练时“并行监督”和推理时“自回归展开”的典型区别。
    """
    ...
    def forward_finetune(
        ...
        **kwargs,
        ...
        # 训练阶段已经知道所有组的位置，因此可以并行地得到完整隐藏状态。
        ...
        for i in range(1, self.config.num_code_groups):
            # 第 i 组 codec 使用独立线性头，反映多码本离散空间之间并不共享分类器参数。
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Prefill 阶段: 输入是完整隐藏序列，模型需要先建立缓存并确定当前生成到了第几组 codec。
        if inputs_embeds is not None and inputs_embeds.shape[1] > 1:
            generation_steps = inputs_embeds.shape[1] - 2  # hidden & layer 0
        # 真正逐 token 生成阶段: 只会拿到当前组的离散 id，需要从对应 embedding 表恢复输入。
        else:
...
class Qwen3TTSTalkerModel(Qwen3TTSTalkerTextPreTrainedModel):
    ...
    AI 视角:
    - 这是文本到语音生成链路中的核心 LM，负责联合建模文本 token、特殊控制 token、
      speaker 插槽以及主 codec 通道。
    - 它产生的隐藏状态既直接用于生成 codec-0，也会作为 sub-talker 的条件输入。

    实现要点:
    - 文本 embedding 与 codec embedding 位于同一隐藏空间，最终会在时间轴上相加融合。
    - 位置编码采用适配多模态场景的 mRoPE 形式，因此 `position_ids` 可能是三维张量。
    """
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # 这里固定展开成 3 个位置轴，是为了兼容多模态 rotary 的接口约定。
        # 对当前 TTS 任务而言，核心仍然是时间轴，但实现复用了更通用的 mRoPE 形态。
        if position_ids is None:
        ...
        # 所有 decoder layer 共享同一组 rotary 位置编码。
        position_embeddings = self.rotary_emb(hidden_states, position_ids)
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    职责:
    - 把文本 embedding 投影到与 codec embedding 相同的隐藏空间。
    - 生成 codec-0 主通道。
    - 调用 `code_predictor` 补全其余 codec 组。

    可以把它理解为:
    “负责生成声学骨架的主语言模型 + 负责补全细节码本的辅预测器”的组合体。
    """
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}
    _pp_plan = {"lm_head": (["hidden_states"], ["logits"])}
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    组件关系:
    - `talker`: 文本与 codec 的主生成网络。
    - `speaker_encoder`: 仅 Base 模型需要，用于从参考音频提取说话人 embedding。
    - `speech_tokenizer`: 负责把参考音频编码成离散 codec，或把生成出的 codec 解码回波形。

    AI 视角:
    - 该类并不直接把文本“回归成波形”，而是先生成离散语音 token，
      再借助 speech tokenizer 的 decoder 重建波形。
    - 这种做法相当于把“高层语音规划”和“底层波形合成”解耦，提升训练与推理稳定性。
    """
    config_class = Qwen3TTSConfig
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not local_files_only and not os.path.isdir(pretrained_model_name_or_path):
            # 主权重之外，仓库还依赖子目录 `speech_tokenizer/*`；
            # 这里显式下载，避免只拿到主模型却缺少声学 tokenizer。
        ...
        # 顶层 TTS 模型在运行时依赖 speech tokenizer 做参考音频编码与最终波形解码。
    ...
    def generate_speaker_prompt(
        ...
        """把外部 prompt 中的说话人向量整理到模型设备和 dtype 上。"""
