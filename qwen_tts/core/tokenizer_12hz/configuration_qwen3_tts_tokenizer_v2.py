...
模块定位:
- 12Hz tokenizer 是 Qwen3-TTS 主模型默认依赖的语音离散化器。
- 该配置文件描述了两部分:
  1. `encoder_config`: 把波形编码成离散码。
  2. `decoder_config`: 把离散码解码回波形。

AI 视角:
- 12Hz 代表更稀疏的时间步率，意味着单位时间内 token 更少，
  更适合让语言模型直接建模语音离散序列。
"""
...
class Qwen3TTSTokenizerV2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 这些速率参数决定了“波形采样点 <-> 离散 token 步数”之间的换算关系，
        # 是理解 tokenizer 时间分辨率的关键。
