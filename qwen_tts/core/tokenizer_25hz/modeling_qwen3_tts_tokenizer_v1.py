...
模块总览:
- 该文件实现 25Hz speech tokenizer。
- 相比 12Hz 版本，V1 更强调“从参考条件重建波形”的经典声码器路线，
  因而出现了 x-vector、reference mel、DiT、BigVGAN 等模块。

阅读提示:
- 如果你想理解“传统声学条件 + 神经 vocoder”的思路，这个文件很有代表性。
- 如果你的目标是理解 Qwen3-TTS 主模型如何直接建模离散语音 token，
  那么 V2/12Hz 会更关键。
"""
...
class Qwen3TTSTokenizerV1Decoder(Qwen3TTSTokenizerV1DecoderPreTrainedModel):
    ...
    数据流:
    - codec + x-vector + reference mel
      -> DiT 生成 mel spectrogram
      -> BigVGAN 把 mel 转成波形
    """
    config: Qwen3TTSTokenizerV1DecoderConfig
    base_model_prefix = "model"
...
class Qwen3TTSTokenizerV1Encoder(Qwen3TTSTokenizerV1EncoderPreTrainedModel):
    ...
    def speech2mel(self, speechs):
        """把原始波形列表转成 mel 谱列表，供 WhisperEncoderVQ 使用。"""
        mels = [
            get_mel_audio(
...
class Qwen3TTSTokenizerV1Model(Qwen3TTSTokenizerV1PreTrainedModel):
    """完整的 25Hz speech tokenizer。

    对外约定:
    - `encode` 除了 `audio_codes`，还会返回 `xvectors` 与 `ref_mels`；
      这正是 25Hz 解码阶段额外需要的条件。
    - `decode` 依赖这些条件重建波形，因此接口比 12Hz 版本更重。
    """
    ...
    def load_encoder_xvector_extractor(self, model_path):
        """加载 ONNX x-vector 提取器，用于从参考语音提取说话人表示。"""
