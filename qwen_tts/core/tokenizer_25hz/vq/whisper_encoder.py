...
"""
Whisper 风格音频编码器及其注意力实现。

模块定位:
- 这是 25Hz tokenizer 编码端的骨干网络。
- 文件中既包含 mel 提取辅助函数，也包含一个适配音频序列打包形式的 Transformer 编码器。

AI 视角:
- 编码器先通过卷积下采样获取局部时频模式，再通过多层自注意力建模长程依赖，
  最终输出适合量化的连续表征。
- 如果安装了 flash-attn，会优先走更高效的变长注意力实现。
"""
...
class MultiHeadAttention(nn.Module):
    ...
    Python 视角:
    - `cu_seqlens` 是 flash-attn 常见的“累计长度”表示法，
      用于把多条变长序列拼成一个扁平张量后仍能恢复各自边界。
    - 如果环境不支持 flash-attn 或 dtype 不合适，代码会自动退回手写 PyTorch 实现。
    """
...
class WhisperEncoder(nn.Module):
    ...
    - log-mel spectrogram
      -> 两层卷积提取局部模式并做时间降采样
      -> 多层 Transformer block 建模上下文
      -> 线性投影到输出维度

    该编码器的输出随后会被 VQ 模块进一步离散化。
    """
    def __init__(
            self,
            n_mels: int,
