...
"""
25Hz tokenizer 中与说话人特征和量化器对接的语音组件。

模块定位:
- 该文件提供三类能力:
  1. mel 谱提取；
  2. x-vector 说话人向量提取；
  3. 基于 WhisperEncoder 的向量量化编码器扩展。

AI 视角:
- 它是 25Hz 路线里“连续语音特征 -> 离散码 + 说话人条件”最关键的桥梁。
"""
...
class XVectorExtractor(nn.Module):
    """基于 ONNX Runtime 的 x-vector 提取器。

    AI 视角:
    - x-vector 是经典 speaker verification 体系里的说话人表征，
      在 25Hz tokenizer 中用来补充全局音色条件。

    工程视角:
    - 这里选择 ONNX Runtime 而不是 PyTorch 模型，通常是为了轻量部署与推理兼容性。
    """
...
class WhisperEncoderVQ(WhisperEncoder):
    """在 Whisper 风格编码器顶部加入向量量化模块。

    作用:
    - 先用 WhisperEncoder 抽取连续声学特征；
    - 再用残差量化器把它们压缩成离散索引；
    - 必要时执行下采样/上采样以匹配目标码率。
    """
    def __init__(
