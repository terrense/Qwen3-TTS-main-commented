...
"""
Qwen3-TTS 的 Gradio 本地演示程序。

模块定位:
- 这是一个“把推理包装层暴露给 Web UI”的适配层。
- 代码的重点不在模型算法本身，而在参数解析、输入清洗、音频显示格式转换、
  以及为不同模型类型动态生成界面。

AI 视角:
- demo 支持三种模型形态: custom voice、voice design、base voice clone。
- 不同模型需要的条件不同，因此 UI 也是按模型能力动态裁剪的。

Python/工程视角:
- 该文件展示了如何把后端推理对象封装到 Gradio callback 中。
- 一些辅助函数例如 `_dtype_from_str`、`_audio_to_tuple`、`_wav_to_gradio_audio`
  本质上都是“边界层适配器”。
"""
